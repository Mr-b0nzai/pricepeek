...
root.iconbitmap("H:\\Users\\josiah\\Documents\\pricepeek\\assets\\logo-pricepeek-mark.ico", default="H:\\Users\\josiah\\Documents\\pricepeek\\assets\\logo-pricepeek-mark.ico")
...
for i in range(1):
    # add PeekPrice logo
    logo_path = Image.open("H:\\Users\\josiah\\Documents\\pricepeek\\assets\\logo-pricepeek-mark.png")
    ...
    def show_login_success(username, password, login_window):
        ...
        if result == True:
            ...
            if not hasattr(login_window, 'login_success'):
                ...
                login_success = customtkinter.CTkLabel(master=login_frame, text='Login failed!', font=("Arial Bold", 20))
                login_success.pack()
    ...
    username = customtkinter.CTkLabel(master=login_frame, text='Username', font=("Arial Bold", 24))
    username.pack()
    username_entry = customtkinter.CTkEntry(login_frame, placeholder_text='username')
    username_entry.pack(pady=10)

    password = customtkinter.CTkLabel(master=login_frame, text='Password', font=("Arial Bold", 24))
    password.pack()
    password_entry = customtkinter.CTkEntry(login_frame, placeholder_text='password', show='*')
    password_entry.pack(pady=10)

    login_btn = customtkinter.CTkButton(master=login_frame, text='Login', command=lambda: show_login_success(username_entry.get(), password_entry.get(), login_window))
...
sidebar_button_1 = customtkinter.CTkButton(sidebar_frame, text='How to Use', command=sidebar_button_event)
sidebar_button_1.grid(row=1, column=0, padx=20, pady=10)
sidebar_button_2 = customtkinter.CTkButton(sidebar_frame, text='Saved Predictions', command=sidebar_button_event)
sidebar_button_2.grid(row=2, column=0, padx=20, pady=10)
sidebar_button_3 = customtkinter.CTkButton(sidebar_frame, text='Premium Feature', command=sidebar_button_event)
...
label_radio_group = customtkinter.CTkLabel(master=radiobutton_frame, text="Number of days to predict:")
label_radio_group.grid(row=0, column=2, columnspan=1, padx=10, pady=10, sticky="")
radio_button_1 = customtkinter.CTkRadioButton(master=radiobutton_frame, text='1 Day', variable=radio_var, value=1)
radio_button_1.grid(row=1, column=2, pady=10, padx=20, sticky="n")
radio_button_2 = customtkinter.CTkRadioButton(master=radiobutton_frame, text='2 Days', variable=radio_var, value=2)
radio_button_2.grid(row=2, column=2, pady=10, padx=20, sticky="n")
radio_button_3 = customtkinter.CTkRadioButton(master=radiobutton_frame, text='3 Days', variable=radio_var, value=3)
...
# my_entry.insert(0)
my_entry.grid(row=3, column=1, columnspan=2, padx=(20, 0), pady=(20, 20), sticky="nsew")
    ...
    if btc_data is not None and 'ticker' in btc_data and btc_data['ticker'] == ticker:
        ...
        try:
            ...
            if btc_data['data'].index[-1] != today:
                ...
                new_row = pd.DataFrame({'Open': [0], 'High': [0], 'Low': [0], 'Close': [0], 'Adj Close': [0], 'Volume': [0]}, index=[today])
...
def create_feature_columns(dataset, y_value):
    ...
    dataset['Prediction'] = dataset[y_value].shift(-int(radio_var.get())) # creating new column called 'prediction' that contains the close price of the next 3 days
...
def prepare_dataset(dataset, y_value='Prediction'):
    ...
    X_fcast = X[-int(radio_var.get()):] # creating the set that we will use to predict the future values
...
def run_preparation():
    try:
        ...
        return X_train_scaled, X_valid_scaled, y_train, y_valid, X_fcast_scaled, datasets
    except:
...
def run_prediction():
    ...
    def run_prediction_thread():
        ...
        spinner = customtkinter.CTkProgressBar(root, height=20, mode="indeterminate")
        spinner.grid(row=4, column=1, columnspan=2, padx=(20, 0), pady=(0, 20), sticky="nsew")
        spinner.start()
        ...
        prediction_low = run_model(model, run_preparation()[0]['Low'], run_preparation()[1]['Low'], 
                                run_preparation()[2]['Low'], run_preparation()[3]['Low'], 
                                run_preparation()[4]['Low'], 'Low', run_preparation()[5]['Low'], 'Low')
        
        prediction_high = run_model(model, run_preparation()[0]['High'], run_preparation()[1]['High'], 
                                    run_preparation()[2]['High'], run_preparation()[3]['High'], 
                                    run_preparation()[4]['High'], 'High', run_preparation()[5]['High'], 'High')
                
        prediction_close = run_model(model, run_preparation()[0]['Close'], run_preparation()[1]['Close'], 
                                    run_preparation()[2]['Close'], run_preparation()[3]['Close'], 
                                    run_preparation()[4]['Close'], 'Close', run_preparation()[5]['Close'], 'Close')

        # concatenate the two dataframes into a single dataframe
        predictions = pd.concat([prediction_low, prediction_high, prediction_close], axis=1)
        print(predictions)

        # Define a function to display the labels
        def display_labels():
            low_label = customtkinter.CTkLabel(root, text=prediction_low, font=("Helvetica", 24, "bold"))
            low_label.grid(row=0, column=1, columnspan=2, padx=(20, 0), pady=(20, 20), sticky="nsew")
            high_label = customtkinter.CTkLabel(root, text=prediction_high, font=("Helvetica", 24, "bold"))
            high_label.grid(row=1, column=1, columnspan=2, padx=(20, 0), pady=(20, 20), sticky="nsew")
            close_label = customtkinter.CTkLabel(root, text=prediction_close, font=("Helvetica", 24, "bold"))
            close_label.grid(row=2, column=1, columnspan=2, padx=(20, 0), pady=(20, 20), sticky="nsew")
...
def run_model(model, X_train_scaled, X_valid_scaled, y_train, y_valid, X_fcast_scaled, y_value, dataset, y_col):
    ...
    d.index = pd.date_range(start=download_dataset(my_entry.get()).index[-1], periods=(len(d)), freq='D')
...
run_btn = customtkinter.CTkButton(master=root, text = "Run Prediction", command = run_prediction)
run_btn.grid(row=3, column=3, padx=(20, 20), pady=(20, 20), sticky="nsew")
